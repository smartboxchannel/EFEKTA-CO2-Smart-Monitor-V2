...
const tzLocal = {
	node_config: {
        ...
        convertSet: async (entity, key, rawValue, meta) => {
            ...
            const payloads = {
                reading_interval: ['genBasic', {0x0201: {value, type: 0x21}}],
            };
...
const definition = {
        ...
        fromZigbee: [fz.temperature, fz.humidity, fzLocal.co2, fzLocal.air_quality, fzLocal.co2_config, fzLocal.temperaturef_config,
		    fzLocal.humidity_config, fzLocal.termostat_config, fzLocal.hydrostat_config, fzLocal.co2_gasstat_config, fzLocal.tvoc_gasstat_config, fzLocal.node_config],
        toZigbee: [tz.factory_reset, tzLocal.co2_config, tzLocal.temperaturef_config, tzLocal.humidity_config,
		    tzLocal.termostat_config, tzLocal.hydrostat_config, tzLocal.co2_gasstat_config, tzLocal.tvoc_gasstat_config, tzLocal.node_config],
		...
		configure: async (device, coordinatorEndpoint, logger) => {
            ...
            minimumReportInterval: 30, maximumReportInterval: 3600, reportableChange: 1}];
            await firstEndpoint.configureReporting('msCO2', payload1);
			const payload2 = [{attribute: {ID: 0x0055, type: 0x39},
            minimumReportInterval: 30, maximumReportInterval: 3600, reportableChange: 1}];
            await firstEndpoint.configureReporting('genAnalogInput', payload2);
			const payload3 = [{attribute: {ID: 0x0000, type: 0x29},
            minimumReportInterval: 30, maximumReportInterval: 3600, reportableChange: 25}];
			await firstEndpoint.configureReporting('msTemperatureMeasurement', payload3);
			const payload4 = [{attribute: {ID: 0x0000, type: 0x21},
            minimumReportInterval: 30, maximumReportInterval: 3600, reportableChange: 50}];
			await firstEndpoint.configureReporting('msRelativeHumidity', payload4);
			const payload5 = [{attribute: {ID: 0x0055, type: 0x39},
            minimumReportInterval: 0, maximumReportInterval: 3600, reportableChange: 0}];
			await secondEndpoint.configureReporting('genAnalogInput', payload5);
        },
        exposes: [e.co2(),
			...
			exposes.numeric('reading_interval', ea.STATE_SET).withUnit('Seconds').withDescription('Setting the sensor reading interval Setting the time in seconnds, by default 30 seconds')
                .withValueMin(30).withValueMax(360),
